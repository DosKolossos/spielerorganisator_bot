...
async function runSundayPlanner(client, options = {}) {
    ...
    const channelId = options.channelId || process.env.ADMIN_CHANNEL_ID;

    console.log('[Planner] ADMIN_CHANNEL_ID:', process.env.ADMIN_CHANNEL_ID);
    console.log('[Planner] effective channelId:', channelId);

    const adminChannel = await client.channels.fetch(channelId);
    if (!adminChannel || !adminChannel.isTextBased()) {
        console.error('[Planner] Adminkanal nicht gefunden oder nicht textbasiert.');
        return;
    }

    await adminChannel.send('Planner-Test erfolgreich erreicht.');
