...
module.exports = {
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addStringOption(option =>
          option
            .setName('wiederholung')
            .setDescription('Optional: neuer Wiederholungstyp')
            .setRequired(false)
            .addChoices(
              { name: 'Wöchentlich', value: 'weekly' },
              { name: 'Alle 2 Wochen', value: 'biweekly' },
        ...
        .addStringOption(option =>
          option
            .setName('typ')
            .setDescription('Optional: neuer Regeltyp')
            .setRequired(false)
            .addChoices(
              { name: 'Nicht verfügbar', value: 'nicht_verfuegbar' },
              { name: 'Erst ab Uhrzeit verfügbar', value: 'erst_ab' },
        ...
        .addStringOption(option =>
          option
            .setName('tage')
            .setDescription('Optional: neue Tage für weekly/biweekly')
            .setRequired(false)
        )
        .addStringOption(option =>
          option
            .setName('uhrzeit')
            .setDescription('Optional: neue Uhrzeit im Format HH:MM')
            .setRequired(false)
        )
        .addStringOption(option =>
          option
            .setName('notiz')
            .setDescription('Optional: neue Notiz, mit "-" wird sie gelöscht')
            .setRequired(false)
        )
    )
    .addSubcommand(sub =>
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'bearbeiten') {
      ...
      const recurrenceInput = interaction.options.getString('wiederholung');
      const ruleTypeInput = interaction.options.getString('typ');
      const startdatumInput = interaction.options.getString('startdatum')?.trim() ?? null;
      const tageInput = interaction.options.getString('tage')?.trim() ?? null;
      const timeInput = interaction.options.getString('uhrzeit')?.trim() ?? null;
      ...
      const recurrenceType = recurrenceInput ?? (existing.recurrence_type ?? 'weekly');
      const ruleType = ruleTypeInput ?? existing.rule_type;

      const parsedStartDate = startdatumInput ? parseDateInput(startdatumInput) : existing.anchor_date;
      ...
      const timeValue =
        ruleType === 'nicht_verfuegbar'
          ? null
          : (timeInput ?? existing.time_value);

      if ((ruleType === 'erst_ab' || ruleType === 'bis') && !timeValue) {
        return interaction.reply({
          content: 'Für diesen Regeltyp muss eine Uhrzeit vorhanden sein.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (recurrenceType === 'weekly' || recurrenceType === 'biweekly') {
        if (tageInput) {
          const parsedMask = parseWeekdayMask(tageInput);
          if (parsedMask === null) {
            return interaction.reply({
              content: 'Ungültige Tage. Nutze z. B. werktage, wochenende, alle oder montag,dienstag.',
              flags: MessageFlags.Ephemeral
          ...
          weekdayMask = parsedMask;
        }

        if (!weekdayMask) {
          return interaction.reply({
            content: 'Für wöchentliche oder zweiwöchentliche Regeln müssen Tage gesetzt sein.',
            flags: MessageFlags.Ephemeral
          });
        }
      } else if (tageInput) {
        return interaction.reply({
          content: 'Für monatliche oder jährliche Regeln nutze bitte kein Feld "tage".',
          flags: MessageFlags.Ephemeral
        });
      } else {
        weekdayMask = 0;
      ...
      const anchorDate = parsedStartDate;
